...
```text
V0.1 验收进行中：7 项通过；真实 K 线与依赖行情源的 Agent 任务待复验
```
...
注：2026-09-26 验收证据见 `docs/acceptance.md`。真实指数、搜索、报价和指数 Agent 曾成功，之后行情连接间歇失败；日 K、周 K 持续返回 503。固定响应测试与 Eval 不替代真实 K 线在线通过，因此前两项保持进行中。
